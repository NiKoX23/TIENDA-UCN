...
export class FacturacionService {
  ...
  async crearCompra(uid: number, dto: CrearCompraDto) {
    if (!dto.lineas?.length) throw new BadRequestException('El carrito está vacío');

    const cantidades = new Map<string, number>();
    for (const linea of dto.lineas) {
      cantidades.set(linea.codigoProducto, (cantidades.get(linea.codigoProducto) ?? 0) + linea.cantidad);
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (cliente) {
        ...
        cliente = clienteRepository.create({ uid, nombre: usuario.nombre, email: usuario.email });
      }
      cliente = await clienteRepository.save(cliente);

      const varianteRepository = manager.getRepository(VarianteProducto);
      const detalles: Array<{ idVariante: number; cantidad: number; precio: number; subtotal: number }> = [];
      for (const [codigoProducto, cantidad] of cantidades) {
        ...
        if (!variante || !variante.producto.activo) throw new NotFoundException(`Producto no encontrado: ${codigoProducto}`);
        if (variante.stock < cantidad) throw new BadRequestException(`Stock insuficiente para ${codigoProducto}`);
        detalles.push({
      ...
      const factura = await facturaRepository.save(facturaRepository.create({
        cliente, tipoDocumento: 'boleta', numeroDocumento, metodoPago: dto.metodoPago, total,
      }));

      const salidaRepository = manager.getRepository(Salida);
      for (const detalle of detalles) {
        const variante = await varianteRepository.findOneByOrFail({ idVariante: detalle.idVariante });
        await salidaRepository.save(salidaRepository.create({
          factura, variante, cantidad: detalle.cantidad, precioUnitario: detalle.precio, subtotal: detalle.subtotal,
        }));
